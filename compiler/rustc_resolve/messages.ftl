...
resolve_ident_imported_here_but_it_is_desc =
    `{$ident}` is imported here, but it is {$desc}

resolve_ident_in_scope_but_it_is_desc =
    `{$ident}` is in scope, but it is {$desc}
...
resolve_note_and_refers_to_the_item_defined_here =
    {$first ->
        [true] {$dots ->
            [true] the {$item} `{$name}` is defined here...
            *[false] the {$item} `{$name}` is defined here
        }
        *[false] {$dots ->
            [true] ...and refers to the {$item} `{$name}` which is defined here...
            *[false] ...and refers to the {$item} `{$name}` which is defined here
        }
    }

resolve_outer_ident_is_not_publicly_reexported =
    {$descr} `{$outer_ident}` is not publicly re-exported
...
resolve_unresolve_import = { $number ->
    ...
    *[other] unresolved imports {$paths}
}
...
resolve_you_could_import_this_desc =
    you could import this {$desc}
